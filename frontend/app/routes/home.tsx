  ...
  return (
    <main className="pt-16 pb-8 px-4">
      <div className="container mx-auto">
        ...
        <form className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 mb-6" onSubmit={(e) => e.preventDefault()}>
          <div className="flex flex-col md:flex-row gap-3 items-center">
            <input
              value={location}
              onChange={(e) => {
          setLocation(e.target.value);
          setQuery({ location: e.target.value || undefined, bedrooms: beds ?? 0, bathrooms: baths ?? 0, amenities: selectedAmenities });
              }}
          ...
          setBeds(val);
          setQuery({ location: location || undefined, bedrooms: val ?? 0, bathrooms: baths ?? 0, amenities: selectedAmenities });
              }}
          ...
          const val = e.target.value === "" ? null : Number(e.target.value);
          setBaths(val);
          setQuery({ location: location || undefined, bedrooms: beds ?? 0, bathrooms: val ?? 0, amenities: selectedAmenities });
              }}
              className="rounded-md border px-3 py-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            >
          ...
          return (
            <button
              ...
              onClick={() => {
                toggleAmenity(a);
                const newAmenities = selectedAmenities.includes(a)
            ? selectedAmenities.filter((am) => am !== a)
            : [...selectedAmenities, a];
                setQuery({ location: location || undefined, bedrooms: beds ?? 0, bathrooms: baths ?? 0, amenities: newAmenities });
              }}
